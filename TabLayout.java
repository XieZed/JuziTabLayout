package com.haokan.yitu.customview.mytab;/*
...
public class TabLayout extends HorizontalScrollView {
    ...
    private int calculateScrollXForTab(int position, float positionOffset) {
        if (mMode == MODE_SCROLLABLE) {
            final View selectedChild = mTabStrip.getTextViewChildAt(position);
            final View nextChild = position + 1 < mTabStrip.getChildCount()
                    ? mTabStrip.getTextViewChildAt(position + 1)
                    : null;
            final int selectedWidth = selectedChild != null ? selectedChild.getWidth() : 0;
            final int nextWidth = nextChild != null ? nextChild.getWidth() : 0;

            int offset = ((ViewGroup)selectedChild.getParent()).getLeft();
            return selectedChild.getLeft() + offset
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        SlidingTabStrip(Context context) {
            ...
            setVerticalGravity(GRAVITY_CENTER);
            setDividerPadding(DisplayUtil.dip2px(getContext(), 10));
            setDividerDrawable(getContext().getResources().getDrawable(com.haokan.yitu.R.drawable.drawable_divider_vertical));
            setWillNotDraw(false);
            mSelectedIndicatorPaint = new Paint();
        }

        //****自定义的部分 begin****
        View getTextViewChildAt(int position) {
            final TabView view = (TabView) getChildAt(position);
            TextView textView = view.mTextView;
            if (textView == null) {
                return this;
            }
            return view.mTextView;
        }
        //****自定义的部分 end****
        ...
        private void updateIndicatorPosition() {
            final View selectedTitle = getTextViewChildAt(mSelectedPosition);
            int left, right;

            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                int selectOffset = ((ViewGroup)selectedTitle.getParent()).getLeft();
                left = selectedTitle.getLeft() + selectOffset;
                right = selectedTitle.getRight() + selectOffset;

                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    // Draw the selection partway between the tabs
                    View nextTitle = getTextViewChildAt(mSelectedPosition + 1);
                    int nextOffset = ((ViewGroup)nextTitle.getParent()).getLeft();
                    left = (int) (mSelectionOffset * (nextTitle.getLeft() + nextOffset) +
                            (1.0f - mSelectionOffset) * left);
                    right = (int) (mSelectionOffset * (nextTitle.getRight() + nextOffset) +
...
//            LogHelper.d("myTabLayout", " left , right = " + left + ", " + right);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            final View targetView = getTextViewChildAt(position);
            if (targetView == null) {
            ...
            int targetOffset = ((ViewGroup)targetView.getParent()).getLeft();
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                canvas.drawRoundRect(new RectF(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight,
                        mIndicatorRight, getHeight()), 2.0f, 2.0f, mSelectedIndicatorPaint);
